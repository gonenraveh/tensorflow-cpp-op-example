...
REGISTER_OP("InnerProduct")
  ...
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
    ...
    TF_RETURN_IF_ERROR(c->WithRank(c->input(0), 2, &input_shape));  // "input"   with [rows=N,cols=1]    
    TF_RETURN_IF_ERROR(c->WithRank(c->input(1), 2, &weight_shape)); // "weights" with [M,N]
...
public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    // check input is a column vector
