...
REGISTER_OP("InnerProductGrad")
  ...
  .Output("grad_input: float32")    // an Op with two inputs, provides two tensor gradients
...
public:
  ...
  void Compute(OpKernelContext* context) override { 
    ...
    auto grad_tensor = grad.matrix<float>();  // incoming gradient tensor for later layer in graph
    auto weights_tensor = weights.matrix<float>();
    auto input_tensor = input.matrix<float>();
    auto grad_input_tensor = grad_input->matrix<float>();     // 1D
    auto grad_weights_tensor = grad_weights->matrix<float>(); // 2D
    // compute gradient for input: dInput = gradient*weights
    ...
    // compute gradient for weights: dW = gradient*input
    for (int i = 0; i < weights_shape.dim_size(0); i++) {
